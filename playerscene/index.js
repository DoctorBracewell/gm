...
const outputEditor = CodeMirror.fromTextArea(
  document.querySelector("#scriptOutput"),
  { theme: "lesser-dark", lineNumbers: true, lineWrapping: true }
);
...
if (typeof Storage !== "undefined") {
  ...
  function openingCode() {
    ...
    for (
      let i = 0;
      i < parseInt(localStorage.getItem("PS-vertices")) - 1;
      i++
    ) {
      newVertex();
    }
    ...
    Object.keys(localStorage).forEach((element) => {
      ...
      if (element.includes("PS-")) {
        document.querySelector(`#${element.replace("PS-", "")}`).value =
...
function deleteAll() {
  let saved = [
    ...
    vertices =
      document.getElementsByClassName("vertex-box").length > 0
        ? document.getElementsByClassName("vertex-box")
        : [];
...
function envelopeFunctions(code) {
  let starters = [
    ...
    document.querySelector("#invisibleTrue").checked
      ? "player.setInvisible(true);\n"
      : "",
    document.querySelector("#startSnippet").value
      ? `${document.querySelector("#startSnippet").value}\n`
      : "\n",
  ];
  ...
  let enders = [
    ...
    document.querySelector("#endSnippet").value
      ? `${document.querySelector("#endSnippet").value}\n`
      : "",
  ];
...
function loopVertices() {
  let vertexArray = [],
    first = document.querySelector(".vertex-box-1");
  vertexArray.push(
    new Vertex(
      {
    ...
    vertexArray.push(
      new Vertex(
        ...
        },
        {
          ...
          direction: i.querySelector(".clockwise").checked
            ? "clockwise"
            : "anticlockwise",
        },
...
function generatePath(vertexA, vertexB) {
  ...
  let distance = {
    ...
    pitch:
      0 - (parseFloat(vertexA.view.pitch) - parseFloat(vertexB.view.pitch)),
  };
  if (vertexB.view.direction === "anticlockwise") {
    distance.yaw = -(
      (360 - parseFloat(vertexB.view.yaw) + parseFloat(vertexA.view.yaw)) %
      360
    );
  }
  if (vertexB.view.direction === "clockwise") {
    distance.yaw =
      (360 - parseFloat(vertexA.view.yaw) + parseFloat(vertexB.view.yaw)) % 360;
  }
  ...
  for (let i = 0; i <= parseInt(vertexB.time) * 25; i++) {
    finishedCode += `player.execute("tp @s ${tracker.x.toFixed(
      3
    )} ${tracker.y.toFixed(3)} ${tracker.z.toFixed(3)} ${tracker.yaw.toFixed(
      1
    )} ${tracker.pitch.toFixed(1)}");\nscript.wait(1);\n`;
