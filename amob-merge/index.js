let moving = CodeMirror.fromTextArea(document.querySelector("#moving"), {
    mode: "yaml",
    theme: "lesser-dark",
    lineNumbers: true,
    lineWrapping: true,
  }),
  attacking = CodeMirror.fromTextArea(document.querySelector("#attacking"), {
    mode: "yaml",
    theme: "lesser-dark",
    lineNumbers: true,
    lineWrapping: true,
  }),
  idle1 = CodeMirror.fromTextArea(document.querySelector("#idle1"), {
    mode: "yaml",
    theme: "lesser-dark",
    lineNumbers: true,
    lineWrapping: true,
  }),
  idle2 = CodeMirror.fromTextArea(document.querySelector("#idle2"), {
    mode: "yaml",
    theme: "lesser-dark",
    lineNumbers: true,
    lineWrapping: true,
  }),
  idle3 = CodeMirror.fromTextArea(document.querySelector("#idle3"), {
    mode: "yaml",
    theme: "lesser-dark",
    lineNumbers: true,
    lineWrapping: true,
  }),
  output = CodeMirror.fromTextArea(document.querySelector("#output"), {
    mode: "yaml",
    theme: "lesser-dark",
    lineNumbers: true,
    lineWrapping: true,
...
let editors = [moving, attacking, idle1, idle2, idle3, output];
...
function merge() {
  ...
  if (animations.length < 2) {
    throw new Error(
      "Not enough animations found to continue. Make sure that there are at least two."
    );
  }
  ...
  for (let s of animations[baseAnimation]) {
      ...
      for (let i = 0; i < 5; i++) {
        if (
          ...
          if (!foundStand && currentDisplayName != undefined) {
            throw new Error(
              "Armor stand with snow tag " +
                currentDisplayName.split(": ").pop() +
                " missing in " +
                inputIDs[i]
            );
          }
    ...
    if (!copyFromDiffrentAnimation) {
      ...
      copyFromDiffrentAnimation = false;
    }
    offset++;
  }
